...
\begin{table}[htbp]
\centering
\caption{Provincial Distribution of Apple Planting Area and Production in China in 2023}
...
Production (Mt) &
Production Share (\%) &
Planting Area (mil. mu) &
Area Share (\%) \\
\midrule
China      & 49.60 & 100.00 & 28.93 & 100.00 \\
Shaanxi    & 13.75 & 27.72  & 8.73  & 30.18 \\
Shandong   & 10.40 & 20.97  & 3.55  & 12.27 \\
Gansu      & 5.12  & 10.32  & 5.80  & 20.05 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\footnotesize
\item Notes: The table reports absolute values (planting area in million mu and production in million metric tons) alongside each province's share of the national total. Source: FAOSTAT and China National Bureau of Statistics
\end{tablenotes}
\end{threeparttable}
\end{table}


While the national figures provide a broad overview of China's apple industry, the regional dynamics are equally important for understanding the intricacies of this market. As shown in Table~\ref{tab: production provinces}, Shaanxi province stands out as a critical player in this context. The phrase ``The world looks to China for apples, and China looks to Shaanxi'' captures Shaanxi Province's prominent role as the world's largest contiguous high-quality apple growing area. Shaanxi Province ranks first in China in both apple planting area and production volume in 2025, identified by the FAO as a core contributor to national output \citep{FAO2004}. Its apples have also gained international recognition through registration as a Protected Designation of Origin in Europe \citep{UKGov2021}. Shaanxi produced 13.75 million tons of apples in 2023, accounting for around 28\% of the national total.

Shaanxi faces significant challenges in the storage and distribution of its apples. As a storable fruit, apples have the potential to extend their market reach beyond the harvest season. But the province's storage capacity, estimated at around 5.8 million tons, is insufficient to meet the demands of local farmers and fruit traders. This shortfall points to an underdeveloped cold storage infrastructure, which affects not only the efficiency of the supply chain but also the income stability of the farmers. This gap in storage capacity is a crucial element to consider in understanding the market dynamics of Shaanxi's apple industry, and it serves as a backdrop for our later analysis of the broader market structure of the fresh apple supply chain.



\section{Import and Export Patterns}
...
\subsection{Leaving the Orchard: Marketing Choices of Apple Growers}
\noindent The marketing timeline of primary growers is as follows: 

\begin{tikzpicture}[
  timeline/.style={draw=black, line width=0.9pt, -{Triangle[length=4pt,width=6pt]}},
  dot/.style={circle, fill=black, inner sep=0pt, minimum size=4pt},
  event/.style={draw, rounded corners, fill=white, align=left, inner sep=6pt, text width=8.2cm},
  tlabel/.style={font=\bfseries, align=right, text width=2.8cm},
  link/.style={draw=black, line width=0.6pt}
]

% Coordinates (more flexible than hard-coding node positions)
\coordinate (T0) at (0,0);
\coordinate (T1) at (0,-2.0);
\coordinate (T2) at (0,-4.4);
\coordinate (T3) at (0,-6.8);
\coordinate (T4) at (0,-9.2);
...
% Time labels (left)
\node[tlabel, left=18pt of T1] {Oct. (Harvest)};
\node[tlabel, left=18pt of T2] {Within 2 weeks};
\node[tlabel, left=18pt of T3] {Jan--Feb};
\node[tlabel, left=18pt of T4] {Apr--June};

% Event boxes (right)
\node[event, right=18pt of T1] (E1) {Harvest and initial marketing begins (field buyers assemble apples).};

\node[event, right=18pt of T2] (E2) {First sell-or-store decision: sell immediately or place apples into cold storage.};

\node[event, right=18pt of T3] (E3) {Mid-season selling decision: choose to liquidate inventory or keep apples in storage.};

\node[event, right=18pt of T4] (E4) {Late-season liquidation: final window to sell stored apples before quality deterioration and the next harvest cycle.};

% Connectors from dots to boxes
\draw[link] (T1) -- (E1.west);
\draw[link] (T2) -- (E2.west);
\draw[link] (T3) -- (E3.west);
\draw[link] (T4) -- (E4.west);
...
\subsection{Connecting the Ends: Distributing Channels of Intermediaries}
...
\subsection{Middlemen Scale and Supply Chain Roles}
...
\subsection{Dynamics of Local and non-local Traders \label{Section: intro of Local and non-local traders}}
\noindent Similar to many agricultural commodity markets, such as California's tomato industry \citep{hamilton2024spatial}, China's fresh-apple procurement markets are highly localized. Although up to 100--150 buyers may appear within a county over the course of the marketing year—including short-term, non-local traders who may operate for only a few days, who are dispersed across more than hundreds of villages. As a result, fewer than one buyer is present per village on average, implying that effective competition is determined primarily at the village level rather than the county level.

These markets are characterized by a limited number of active buyers at the point of transaction, together with substantial frictions arising from costly cross-region transportation and the perishability of fresh apples. Such conditions create scope for localized buyer power and coordinated behavior. Consistent with this structure, most intermediaries can be classified into two distinct groups: ``local traders,'' who operate repeatedly within a small set of villages, and ``non-local traders,'' who enter temporarily during the marketing season.

``Local traders'' refer to merchants based in the same region, whose primary trading activities are also concentrated within the region. In contrast, ``non-local traders'' are merchants from other regions who specifically travel to the region to purchase apples.  
...
\section{Roles of Storage \label{Section: role of Storage}}
...
\subsection{Storage Pricing Structure: Based on Quantity, Not Duration} 
...
\subsection{Shortage and Inefficient Allocation in Cold Storage Infrastructure} 
...
\section{Apple Futures Market: The World's Only Fresh-Fruit Futures}
\noindent
Launched on the China Zhengzhou Commodity Exchange (CZCE) in 2017, China's fresh apple futures market is the world's only futures market dedicated to fresh fruit. As China accounts for more than half of global apple production, the contract was introduced to address substantial price volatility and to provide risk management tools for market participants.

Unlike most agricultural futures contracts, which are based on shelf-stable commodities, the apple futures contract is written on fresh apples. Delivery is restricted to the Red Fuji variety and requires strict quality, grading, and storage standards consistent with both domestic and export markets. These requirements distinguish apple futures from traditional grain and oilseed contracts and substantially limit participation to a small subset of large traders, cooperatives, and storage operators.

Importantly, participation by small-scale farmers remains extremely limited even in 2025. Contract sizes, margin requirements, quality certification, and delivery logistics constrain direct farmer access to futures markets. As a result, most farmers do not hedge directly but instead continue to transact almost exclusively in local spot markets with itinerant traders.

Moreover, while futures markets provide a mechanism for hedging aggregate price risk, they do not eliminate localized buyer power. Farm-gate transactions occur at prices that reflect the futures price net of basis, where basis varies spatially with transportation costs, storage access, and local buyer concentration. Consequently, even when futures prices are observable in town, farmers remain exposed to local market conditions that determine the effective price received.

Storage therefore continues to play a central economic role. Because most farmers cannot deliver into futures contracts, intertemporal arbitrage is conducted through physical storage rather than through financial hedging. The decision to store apples represents a choice to delay selling into future spot markets, where prices remain negotiated with buyers whose market power may vary over time. In this sense, futures markets hedge aggregate price risk but do not substitute for competitive access to downstream buyers.
...
This leads to a central question: Who really profits from China's apples? In 2020, an informal investigation by China's Ministry of Agriculture traced the journey of apples from orchards in Shaanxi to retail shelves in Shanghai---a 3,400-kilometer journey that saw prices increase more than fivefold. Initially, a field buyer purchased 3 kilograms of apples directly from farmers for 10 RMB. After transportation to Jiaxing and accounting for logistics costs, 10 RMB bought only 2 kilograms. Following additional sorting and distribution through wholesale markets, the same amount fetched just 1 kilogram. Finally, in Shanghai supermarkets, 10 RMB was exchanged for a mere 0.5 kilograms. This example underscores critical questions about the distribution of value along the supply chain and the competitive forces, or lack thereof, that shape outcomes for stakeholders.

Based on my fieldwork in 2023 and 2024, the apple market in Northwest China operates as a buyer's market. Apple farmers (sellers) function as perfect competitors, while intermediaries (buyers) often exhibit collusive behavior. While some argue that farmers could secure better prices through cooperatives, most local cooperatives are too small in scale compared to buyers' procurement volumes and face significant competition among themselves. Additionally, cooperatives are loosely organized, leaving most farmers to sell individually. Buyer competition, however, is closely tied to downstream demand. During periods of low demand, field buyers rarely compete, but in times of strong demand, intensified competition for market share drives up farm-gate prices.

This section examines the market structure and competitive dynamics in the upstream segment of China's fresh apple supply chain, with particular attention to buyer power. It addresses three core questions: (i) does collusion among buyers exist, and, if so, in what forms; (ii) why do competitive conditions among buyers vary across periods; and (iii) what role does storage play in mitigating buyer power. The analysis builds on descriptive micro-level evidence collected during fieldwork in Yanchang County, Shaanxi Province.
...
\noindent The existence of market territories among buyers in Yanchang County is a subject of debate. Although field-buyer cartels generally do not cooperate, traders within a given cartel coordinate their procurement areas, establishing informal market territories to avoid internal conflicts. For instance, Buyer A in LeiJiaTan Town noted: 
...
This overlapping of procurement territories among cartels demonstrates a dual dynamic: internal coordination fosters monopsony market power within cartels, while external competition between cartels introduces inter-group rivalry, reducing market concentration. 
...
\paragraph{``More but Dispersed'' or ``Less but Regulated''?} 
...
\paragraph{Breach of Implicit Contracts:}
...
\section{Inefficient Storage and Competition Dynamics}


\paragraph{Impacts of Storage:} Storage plays a pivotal role in shaping the dynamics of the apple supply chain. Storage significantly influences both horizontal and vertical competition in the market.
...
\begin{enumerate}
    \item storage improves farmers' outside options by reducing the marginal cost of waiting. Stored apples can be marketed over an extended period, allowing farmers to delay sales, contact additional buyers, or sell to downstream traders operating at later stages of the supply chain. As a result, farmers are no longer tied to immediate transactions only with local field buyers.
...
\paragraph{Inefficiency of Storage Adoption:}
...
At the village level, monopsony or oligopsony power held by dominant buyers enables them to dictate prices and trading terms. During the harvest season, these powerful intermediaries may suppress purchase prices, exploiting farmers' limited bargaining power and lack of access to alternative buyers. Consequently, also observed by \cite{jin2024losses}, apple growers in China often store their produce as a defensive strategy, hoping for better prices later. Whether such farmer-level storage is economically viable, however, is theoretically ambiguous. Storage can raise expected returns when buyer power decreases in the future or future prices improve sufficiently, but its profitability depends critically on storage efficiency, costs, and farmers' attitudes toward risk. In settings where smallholders rely on decentralized, high-cost storage technologies, returns may be eroded; in contrast, when storage losses are limited and farmers are not excessively risk-averse, storage can constitute a rational response to buyer power. Understanding the conditions under which storage mitigates or exacerbates inefficiencies is therefore central to the analysis in following chapters.
