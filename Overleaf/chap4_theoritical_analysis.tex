...
The CE-Gain measure reflects not only higher expected incomes from storage, but also the smoothing of revenue risk that risk-averse farmers value. While the income-gain metric captures the scale of revenue improvements, the CE-Gain embeds how these improvements translate into utility under heterogeneous farmer risk preferences.
...
The result is a potential inverted-U relationship between $G(\mu)$ and $\mu$, driven by the interaction of the \emph{convex} price mapping $p(\theta)$ and (b) the \emph{compactness} of the Beta law induced by $(\mu,\sigma^2)$.
...
%----------------------------------------------------%
\subsection{Welfare Effects of Storage Subsidies by Initial Efficiency}
\noindent The preceding analysis has shown that policies or investments enabling storage generate substantial welfare gains for farmers facing time-varying oligopsonistic power. In practice, the more common constraint is not a complete absence of storage facilities---as in remote interior villages---but rather the presence of storage at relatively high composite costs. These costs reflect not only physical deterioration but also discounting, transportation, and other frictions, resulting in low effective storage efficiency.

A natural policy response is to subsidize storage on a per-unit basis, thereby reducing farmers' perceived cost of holding produce across periods. However, the welfare impacts of such subsidies are heterogeneous. With the same per-unit subsidy, gains could differ systematically depending on farmers' initial storage efficiency and risk preferences.
...
The decomposition of welfare gains across successive improvements in storage efficiency $\kappa$ in Figure~\ref{fig:storage_subsidy_gain_decomposition} reveals a clear pattern of increasing marginal benefits. When $\kappa$ is already relatively high, such as in the $0.90 \rightarrow 0.95$ contrast, the incremental welfare gains are larger than in the low-efficiency step $0.75 \rightarrow 0.80$. This convexity arises because higher storage efficiency strengthens both the intensive and extensive margins of storage. Farmers who were previously indifferent or weakly storing increase their storage share, while those already storing gain more from improved efficiency, so the aggregate response magnifies the value of each incremental subsidy.

At lower baseline levels of $\kappa$, the welfare improvements are more modest in total magnitude but display stronger insurance effects. In the $0.75 \rightarrow 0.80$ step, for example, the reduction in risk premium ($- \Delta RP$) is relatively more pronounced and concentrated among higher-$\gamma$ farmers, highlighting that early-stage subsidies are particularly valuable as insurance instruments. These farmers value the risk-smoothing properties of storage more than the modest gains in mean returns. By contrast, once $\kappa$ reaches higher levels, additional subsidies reduce little risk and instead raise welfare almost exclusively through income growth. 

Taken together, the figure suggests a dual policy message: targeting low-$\kappa$ environments is most effective if the policy priority is insurance for the risk-averse, while concentrating subsidies at already high $\kappa$ levels delivers the greatest marginal aggregate welfare gains by amplifying expected returns. This heterogeneity underscores the importance of targeting and calibrating storage subsidies to local conditions rather than adopting a one-size-fits-all approach.
...
\noindent
The preceding visualizations highlighted the relationships between storage efficiency, buyer power, and welfare outcomes. Table~\ref{tab:mu_cases_storage_outcomes} complements those insights by providing direct estimates for a non-favorable case in which buyer power in both periods is drawn from the same distribution, $\mu_1=\mu_2 \in {0.2,0.5,0.8}$. Even under this relatively restrictive environment, where intertemporal arbitrage opportunities are limited to variance rather than shifts in the mean, the positive role of storage for farmers is evident across welfare metrics.
