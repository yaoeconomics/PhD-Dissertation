...
In practice, I implement four subsidy experiments corresponding to incremental increases in $\kappa$: from 0.75 to 0.80, from 0.80 to 0.85, from 0.85 to 0.90, and from 0.90 to 0.95. For each experiment, I compute $\Delta \bar\pi$, $\Delta \text{CE}$, and $\Delta \text{RP}$ across the entire belief sweep in $\mu$. Graphical presentation reports three curves: gains in expected income, gains in certainty equivalent, and the insurance component (plotted as $-\Delta \text{RP}$ so that all contributions to welfare are positive). Complementary figures decompose these gains by $\gamma$-decile, revealing that the relative importance of insurance rises monotonically with risk aversion. Low-$\gamma$ farmers benefit mostly through growth, while high-$\gamma$ farmers value the reduction in uncertainty.
