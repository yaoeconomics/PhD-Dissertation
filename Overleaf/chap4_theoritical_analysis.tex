...
Building on \citet{suri2011selection}'s framework of self-selection and comparative advantage in technology adoption, storage adoption can be interpreted as the outcome of heterogeneous net returns across farmers. Each farmer faces a unique combination of risk preference, storage efficiency, and market access that determines whether adoption is profitable. Hence, observed variation in storage behavior need not imply inefficiency or constraint; rather, it reflects rational self-selection given heterogeneous comparative advantages.
...
\noindent To examine how farmers adjust their optimal storage share $s^*$ in response to expectations about future buyer power, I simulated a two-period storage model under the same baseline parameters used in the preceding 3D graphics. The first-period buyer power was fixed at $\theta_1 = 0.5$ and storage efficiency at $\kappa = 0.9$. The second-period buyer power, $\theta_2$, followed a Beta distribution on $[0,1]$ with a constant variance of $0.02$ and a mean varying from $0.05$ to $0.95$. For each mean value, I derived the corresponding Beta parameters and generated 5000 random draws of $\theta_2$. The optimal storage share was then obtained by maximizing expected utility under CRRA preferences across five levels of risk aversion, $\gamma \in \{0, 0.5, 2, 4, 7\}$. Figure~\ref{Figure: sensitivity to second-period buyer power} plots the resulting functions $s^\star(\mathbb{E}[\theta_2])$, with line color darkening in proportion to risk aversion.
...
\label{Figure: sensitivity to second-period buyer power}
    ...
    \footnotesize
    \item \textit{Notes:} This figure illustrates how the optimal storage share $s^*$ varies with the expected second-period buyer power $\mathbb{E}[\theta_2]=\mu$ under different levels of risk aversion. 
...
The results in Figure~\ref{Figure: sensitivity to second-period buyer power} highlight three mechanisms. First, under risk neutrality, the decision rule is binary: store everything if $\kappa\,\mathbb{E}[p_2] > p_1$ and sell everything otherwise. In our calibration with $\theta_1 = 0.5$ and $\kappa = 0.9$, the expected-gain curve in Figure~\ref{Figure: expected price gain vs mu} crosses zero at approximately $\mu \approx 0.365$, where $\kappa\,\mathbb{E}[p_2] = p_1$. To make this threshold explicit, define the \emph{indifference buyer-power level} $\theta_2^*(\theta_1)$ by equating the discounted future price to today’s price:
\begin{equation}
\kappa\cdot\frac{1}{1+\theta_2^*}=\frac{1}{1+\theta_1}
\quad\Longrightarrow\quad
\boxed{\theta_2^*(\theta_1)=\kappa\theta_1+\kappa-1.}
\end{equation}
In this case, $\theta_2^*(0.5)=0.35$, implying that storage becomes optimal only when the expected second-period buyer power falls below 0.35. 

Under uncertainty, however, the relevant cutoff expressed in terms of the \emph{mean} buyer power will be slightly higher. Because $p(\theta)=1/(1+\theta)$ is decreasing and convex, Jensen’s inequality gives $\mathbb{E}[p_2] \ge 1/(1+\mathbb{E}[\theta_2])$. Dispersion in $\theta_2$ therefore raises the expected future price for any given mean. To maintain the same indifference condition $\kappa\,\mathbb{E}[p_2] = p_1$, the mean buyer-power threshold must increase---that is, $\mathbb{E}[\theta_2] > \theta_2^*(\theta_1)$---and the gap widens with the variance of $\theta_2$. 

Second, since $p(\theta)$ is convex, the gap $\theta_1-\theta_2^* = (1-\kappa)(1+\theta_1)$ narrows as first-period buyer power weakens, confirmed by observing the changes across three curves in Figure~\ref{Figure: expected price gain vs mu}. This reflects diminishing marginal price gains from further reductions in buyer power, and thus a weaker incentive to store when market power is already low.


Third, as risk aversion rises, the sharp step of the risk-neutral policy becomes a smooth, decreasing $s^*(\mu)$. Higher $\gamma$ lowers $s^*$ point-wise and shifts the effective ``cutoff'' rightward---a higher $\mu$ is required to rationalize the same storage share. Intuitively, downside risk in $p_2$ weighs more heavily under CRRA preferences, so farmers demand a larger expected price advantage (relative to $p_1$) before committing to storage. This is evident in Figure~\ref{Figure: sensitivity to second-period buyer power}: darker lines (higher $\gamma$) lie uniformly below lighter ones, and the onset of interior solutions occurs at higher $\mu$.
...
\begin{figure}[ht!]
    ...
    \begin{tablenotes}[flushleft]
    \footnotesize
    \item \textit{Notes:} solid lines show the absolute gap $\mathbb{E}[\pi(s^*_{RN})]-\mathbb{E}[\pi(s^*_{RA})]$ (left axis). 
...
\paragraph{Why discuss the Cournot case in addition to Bertrand?}  
Following the logic of \cite{kreps1983quantity}, introducing a preliminary stage in which traders receive and commit to fixed ``downstream orders'' (i.e., capacity constraints) transforms the nature of competition. Even if traders subsequently engage in Bertrand-style price competition, the presence of capacity limits prevents them from serving the entire market, and equilibrium outcomes converge to those of a Cournot game. In other words, when capacities are pre-determined and publicly observed, ``capacity choice + Bertrand pricing'' is strategically equivalent to Cournot competition.  
Discussing both cases is therefore necessary: the Bertrand benchmark captures unconstrained price competition among buyers, while the Cournot setting provides a more realistic counterpart where traders' purchasing or storage capacities restrict how aggressively they can bid, yielding a softer form of competition consistent with many agricultural procurement markets.
