...
\end{equation}
I assume farmer risk preferences follow CRRA:
\begin{equation}
    U(\pi)=\left\{
    \begin{array}{ll}
    \pi & \text { if } \gamma = 0 \\
...
\noindent Results in the main text assumed a constant farm supply elasticity, $\varepsilon_1 = \varepsilon_2 = 1$ across both periods. In this appendix, I first consider how results are impacted when the farm supply elasticity changes across periods, considering possibilities that period 2 supply is less elastic, $\varepsilon_1 > \varepsilon_2$ or more elastic, $\varepsilon_1 < \varepsilon_2$, than harvest period elasticity. I then consider impacts when $\varepsilon_1 = \varepsilon_2$ but the common supply elasticity is greater or less than the unitary elasticity assumed in the main text.
...
\noindent
Whereas buyer competition was represented flexibly in the main text based on values for the parameter $\theta$ consistent with the FOOM model, in this appendix I recast the model using the baseline models of noncooperative behavior from the industrial organization literature, symmetric Cournot-Nash (quantity) competition and Bertrand-Nash (price) competition. Both models allow buyers to exercise unilateral market power, but do not consider the possibility of buyer collusive behavior.
...
Because all farmers would sell immediately when multiple buyers are present at harvest ($p_1 = 1$), I focus on the monopsony case with $p_1 = 0.5$. The farmer's problem in Equation~(\ref{eq:final objective}) then specializes to
\begin{equation}
...
Local comparative statics with respect to storage efficiency $\kappa$ and the monopsony probability $\rho$ follow directly from the implicit function theorem applied to $g(s^*)=0$. Because $\partial g/\partial s = d^2\psi/ds^2<0$, I have
$$
...
$$
I aggregate to the village mean in each world, $\bar y_r=\frac{1}{N}\sum_i y_{ir}$, $N=100$. The no-storage benchmark in world $r$ is $\bar y^{\,\text{no}}_r=\frac{1}{N}\sum_i p(\theta_{1r})$.



\paragraph{Income gain metric.}  
To quantify the effect of storage on aggregate revenues, I first construct a percentage income gain measure. For a given environment $(\kappa,\sigma^2,g)$ and initial mean $\mu_1$ buyer power, the expected proportional increase in average income with storage relative to the no-storage benchmark is:
\begin{equation}
...
Because risk preferences are central to welfare, I complement income-based comparisons with a certainty-equivalent (CE) measure derived from CRRA utility. For farmer $i$, expected utility is $\mathrm{EU}_i=\frac{1}{R}\sum_{r=1}^R U(y_{ir};\gamma_i)$, and the corresponding CE income follows from the inverse function of CRRA utility,
\begin{equation}
\mathrm{CE}_i=
\begin{cases}
...
\begin{enumerate}
    ...
    With high storage efficiency and large uncertainty, the relationship between $\mu_1$ and income gains becomes non-monotonic, generating an inverted-U shape.\footnote{The economic and mathematical underpinnings of the inverted-U in Figure~\ref{fig: Income gains} are discussed in Section~\ref{Section: Why Inverted U shapes in some Expected-Income-Gain Curves?}.} Moderate buyer power creates the greatest scope for welfare improvements, while very low or very high buyer power dampens the benefit of storage.
...
\subsubsection{Why Do Some Expected--Income--Gain Curves in Figure~\ref{fig: Income gains} Exhibit an Inverted U?}
\label{Section: Why Inverted U shapes in some Expected-Income-Gain Curves?}

\noindent
Under risk neutrality ($\gamma=0$), the storage decision is binary: the farmer stores if
$$
\kappa\,\mathbb{E}[p(\theta_2)] \;>\; p(\theta_1),
\qquad 
\theta_1,\theta_2\stackrel{i.i.d.}{\sim}\mathrm{Beta}(\alpha(\mu),\beta(\mu)),
$$
with $(\alpha(\mu),\beta(\mu))$ pinned down by the mean $\mu$ and a fixed variance $\sigma^2$. Define
$$
A(\mu)\equiv \kappa\,\mathbb{E}_{\mu}[p(\theta)],
\qquad 
P_1=p(\theta_1)\sim F(\cdot;\mu),
$$
so that the expected gain from storage can be written as
$$
G(\mu) 
= \int_{0}^{A(\mu)} (A(\mu)-p)\,f(p;\mu)\,dp
= A(\mu)\,F(A(\mu);\mu)
  - \int_{0}^{A(\mu)} p\,f(p;\mu)\,dp.
$$

\paragraph{Two forces behind the inverted--U pattern.}
Differentiating under the integral sign yields:
$$
\frac{dG}{d\mu}
= A'(\mu)\,F(A(\mu);\mu)
+ \int_{0}^{A(\mu)} \bigl(A(\mu)-p\bigr)\,\frac{\partial f(p;\mu)}{\partial \mu}\,dp.
$$
Two opposing channels determine the sign of $dG/d\mu$.

\paragraph{(i) Curvature (marginal-gain) channel.}
The mapping $p(\theta)=1/(1+\theta)$ is decreasing and convex ($p'(\theta)<0$, $p''(\theta)>0$). Improvements in buyer competition (lower $\theta$) therefore generate larger price gains when $\theta$ is already low. As $\mu$ increases, the Beta distribution shifts toward higher buyer power, lowering $\mathbb{E}[p(\theta)]$ and hence $A(\mu)$; formally $A'(\mu)<0$, which pushes $G(\mu)$ downward.

\paragraph{(ii) Compactness (probability-mass) channel.}
Holding variance $\sigma^2$ fixed, the Beta density is tight near the boundaries (very low or very high $\mu$) and more diffuse at intermediate $\mu$. A more diffuse distribution for $P_1$ places more probability mass below the cutoff $A(\mu)$, raising the likelihood that $p(\theta_1)<A(\mu)$ and increasing expected gains. This channel pushes $G(\mu)$ upward as $\mu$ moves from low toward moderate values.

\paragraph{Putting the channels together.}
At low $\mu$, curvature strongly favors storage but compactness suppresses it: the marginal payoff from further improvements is high, yet the probability of a substantially better $\theta_2$ is small. As $\mu$ rises to moderate values, the curvature effect weakens but the compactness effect strengthens, so the higher likelihood of improvement outweighs the smaller marginal payoff and $G(\mu)$ increases. At high $\mu$, both forces reduce gains: $A(\mu)$ collapses and the Beta density re-concentrates at high $\theta$, making improvements both unlikely and small, so $G(\mu)$ falls. Together, these forces generate the inverted-U relationship between $G(\mu)$ and $\mu$.

\paragraph{Role of storage efficiency $\kappa$.}
Because $A(\mu)$ scales linearly with $\kappa$, higher storage efficiency uniformly enlarges the truncated area defining $G(\mu)$ and strengthens the inverted-U pattern. When $\kappa$ is low, $A(\mu)$ is compressed toward zero, flattening or eliminating the inverted-U shape.
