...
\noindent In the quantity-setting environment, each homogeneous buyer chooses the quantity to purchase simultaneously, taking other buyers' quantities as given. The resulting equilibrium reflects strategic quantity-setting behavior and corresponds to a Cournot--Nash equilibrium. Unlike the Bertrand case, equilibrium prices are not polarized but vary smoothly with the number of active buyers.


Given this relationship, and maintaining the same linear demand structure used in Section~\ref{Section: Middlemen Market Structure and Farm-Gate Price Formation}, the resulting equilibrium price in each period $t$ under quantity-setting competition is:
\begin{equation}
    p_t = \frac{N_t}{N_t + 1}, \quad t = 1,2.
\end{equation}
where $N_t$ is within the range of positive integers, and hence the bound of $p_t$ is $[0.5, 1)$. This expression captures the inverse relation between market structure conditions and farm-gate prices: as $N_t$ increases, buyer power declines, and the price approaches the competitive limit of 1. Conversely, when $N_t$ is small, buyer power is significant, and prices are suppressed. 

I abstract from the market-failure case of $N_t = 0$, as producers typically retain fallback options—such as self-consumption or sales to processing firms that accept all quality levels at deeply discounted prices. While Figure~\ref{Figure: buyer count at harvest} does include some reported instances of zero buyers, these likely reflect limitations in my survey coverage, which targeted 15 key buyers but did not fully enumerate all potential buyers active in the county.


\paragraph{Maintained Assumption.}  
Throughout the analysis I impose $N_t \geq 1$, ruling out the degenerate case of no buyers. In practice, farmers always face at least one procurement channel (e.g., processors who accept all quality levels at deeply discounted prices). The few instances of $N_t=0$ reported in Figure~\ref{Figure: buyer count at harvest} are best interpreted as artifacts of survey coverage: the data collection targeted 15 key buyers in this county and did not enumerate all possible participants. In the empirical analysis, I verify robustness by (i) dropping these observations and (ii) recoding them to $N_t=1$ when auxiliary sources confirm the presence of a fallback procurement channel.
...
Let's model the second-period buyer count $N_2$ as a discrete Poisson-distributed random variable truncated to the interval $[1,9]$, capturing uncertainty in buyer presence.  I fix first-period buyer count at $N_1 = 3$ as the baseline. As in Section~\ref{Section: Base Model Numerical Analysis}, farmers are assumed to have constant relative risk aversion (CRRA) preferences.
