...
\noindent This study surveys both farmers and intermediaries in Yanchang County, Shaanxi Province, a region where Fuji apple cultivation dominates (see Figure \ref{Figure: Yanchang}). Given its reliance on a single cash crop and the strategic role of cold storage in marketing decisions, Yanchang provides an concrete setting to analyze the interaction between storage and market competition. Each village has a central market where farmers sell apples to middlemen and traders, with commercial cold storage facilities, in either village or the county's downtown, available for rental. In 2024, the county's fruit storage capacity reached 180,000 tons, with most of it utilized annually.  
...
\noindent This study examines apple growers' cold storage usage and marketing timing decisions as key dependent variables. First, I define \textsf{Storage-Usage-Binary}, a binary indicator equal to 1 if a farmer used cold storage at harvest and 0 otherwise. Second, \textsf{Storage-Usage-Type} categorizes storage choices into four groups: \textit{Not\_use} (no storage), \textit{Rent\_large} (rented large-scale commercial storage), \textit{Rent\_small} (rented small-scale storage), and \textit{Self\_built} (self-owned storage). Third, \textsf{Time-to-Sell} measures the delay between harvest and sale, effectively capturing storage duration. Sales occurring in October are classified as 1-4 weeks post-harvest, November as 5-8 weeks, and so forth, yielding interval-censored data with four predefined selling-time intervals: 1-4, 5-8, 9-12, and 13-28 weeks. Figure \ref{Figure: selling weeks distribution} illustrates the distribution of selling weeks by storage type.  
...
\begin{enumerate}
    \item Their subjective assessment of buyer competition at harvest (\textsf{Subjective buyer competitiveness}, measured on a 1-5 scale);
    \item Their expectations regarding changes in the number of traders over the next three months (recorded as \textsf{Expect more buyers} and \textsf{Expect fewer buyers}).
\end{enumerate}
...
\end{enumerate}
These responses allow for the construction of an objective measure of buyer concentration which might serve as a proxy of inter-temporal buyer competition. Inspired by \cite{macchiavello2021competition}, who used the number of coffee mills within a 10 km radius as a competition metric in Rwanda's coffee industry, I define \textsf{Number of buyers} as the count of traders visiting each village at harvest.  
...
\noindent The binary variable \textsf{Liquidity constrained} equals 1 if a farmer reports both difficulty accessing credit and an urgent need to sell at harvest to cover immediate expenses (e.g., school tuition or medical costs). This measure refines and extends approaches used by \cite{albuquerque2024market} and \cite{stephens2011incomplete}, who define credit access as a proxy for liquidity constraints.  
...
\subsection{Farmer Storage Decisions: Buyers' Competitiveness at Harvest}
\noindent 
...
\input{tables/buyer's_competition_logistic_results_AME}
...
\subsection{Farmer Storage Decisions: Farmers' Expectations of Buyer Competitiveness}
...
\begin{equation}
    \text{logit} \left( D(S_i = 1) \right) = \alpha_0 + \alpha_1 E^m_i + \alpha_2 E^f_i + \alpha_3 X_i + \gamma_\tau + \varepsilon_i,
\end{equation}
...
\input{tables/farmer_expectations_logistic_results_AME}


The average marginal effects in Table \ref{tab: binary storage ~ farmer's expectation on movement (AME)} show that farmers who expect an increase in the number of buyers within three months are significantly more likely to store their harvest. The coefficient for \textsf{Expect More Buyers} is 0.325 ($p < 0.01$), suggesting that farmers who anticipate more buyers have a 32.5 percentage point higher probability of using storage. This finding aligns with the idea that expectations of increased buyer competition motivate farmers to delay sales, anticipating higher farm-gate prices. Such behavior is consistent with intertemporal optimization, where farmers strategically store their produce to maximize returns when expecting stronger buyer competition.  

On the other hand, \textsf{Expect Fewer Buyers} yields a positive but statistically insignificant marginal effect of 0.044 ($p = 0.33$), indicating that expectations of reduced competition do not significantly influence storage decisions. While one might expect pessimistic market expectations to discourage storage, the more plausible explanation for the null result is limited variation in this variable. As shown in Table~\ref{tab: summary statistics}, only 16\% of storage users (about 30 farmers) reported expecting fewer buyers, which restricts statistical power. In contrast, expectations of more buyers were far more common, providing sufficient variation to generate a meaningful estimate. The small number of pessimistic respondents thus likely explains the lack of significance.

These findings highlight an asymmetry in farmers' responses to expected changes in buyer competition. While the expectation of more intense competition strongly incentivizes storage, expectations of fewer buyers do not significantly deter it. In the theoretical model from Chapter 4, storage incentives arise from multiple channels. In the absence of temporal changes in buyer competition, farmers with access to sufficiently efficient storage are indifferent between selling immediately and storing, as equilibrium prices rise exactly enough to offset storage costs. By contrast, inefficient storers would strictly lose from storage under purely competitive price dynamics. Viewed through this lens, the empirical asymmetry here reflects the coexistence of multiple storage motivations. Expectations of stronger future competition amplify storage incentives, but they operate alongside efficiency-based considerations and beliefs about future market opportunities, rather than acting as the sole driver of farmers’ storage behavior.
...
These patterns highlight the critical role of forward-looking market expectations in farmers' storage decisions. Figure~\ref{Figure: Difference-from-Baseline Plot} visually illustrates these differences, showing that the increase in predicted storage probability for farmers expecting more buyers is both statistically significant and economically meaningful.\footnote{Figure~\ref{fig:overall_predicted_probs} reports the average predicted probability of storage for each expectation category, evaluated at mean values of continuous covariates and averaged across towns and household characteristics. The red dotted line represents the storage adoption ratio of the whole sample, 0.364. Figure~\ref{fig:predicted_probs_by_family_leadership} disaggregates these predictions by family leadership status, illustrating that households with prior village leadership experience exhibit systematically different baseline storage propensities. Figure~\ref{fig:predicted_probs_by_town} further decomposes predicted probabilities by town, revealing substantial spatial heterogeneity in baseline storage behavior, yet a broadly consistent pattern in how expectations about future buyer competition shifts storage decisions.}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \caption{}
        \label{fig:overall_predicted_probs}
    \end{subfigure}\\[2mm]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \caption{}
        \label{fig:predicted_probs_by_family_leadership}
    \end{subfigure}\\[2mm]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \caption{}
        \label{fig:predicted_probs_by_town}
    \end{subfigure}
...
The vector $\mathbf{x}_{f}$ includes explanatory variables that influence the timing of sales. The survival function $S_j(t)$, which represents the probability that a farmer has not yet sold their apples by time $t$, is related to the hazard function through:
...
\noindent 
Table \ref{tab: extended Survival AFT with controls} presents the results from the AFT Weibull survival model, which estimates the duration (storing time) until farmers sell their apples. Column (1) includes the full sample, while Column (2) focuses exclusively on farmers who utilize storage. The key result found here is that optimistic expectation of buyer competition does lead farmers to store, while does not have estimating power in how long the produce would be in storage.

\begin{enumerate}
    \item \textbf{Expected Buyer Competitiveness and Time to Sell:} Farmers' expectations about future buyer competition exhibit distinct effects on their marketing timing. In the full sample (Column 1), anticipating more buyers in the future significantly prolongs the time to sell ($\beta = 0.50, p<0.01$), suggesting that farmers expecting increased competition among buyers delay sales to later periods through storage, in anticipation of higher farm-gate prices. This aligns with the theoretical expectation that greater future buyer competition incentivizes farmers to wait for more favorable market conditions.
