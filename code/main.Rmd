...
## Pie chart of storage users' type
```{r pie chart}
# Group by storage type and get counts
storage_counts <- df %>%
  group_by(cold_storage_usage_type) %>%
  summarise(count = n())

# Create a pie chart using ggplot2
pie_chart <- ggplot(storage_counts, aes(x = "", y = count, fill = cold_storage_usage_type)) +
  ...
  geom_text(aes(label = paste0(cold_storage_usage_type, ' (', count, ')')),
            position = position_stack(vjust = 0.5))

# Save the plot to a file
ggsave(here("figures", "pie_chart_storage_use_type.png"), pie_chart, width = 12, height = 8)
print(pie_chart)
```
