...
# Ensure data_df exists; if not, load again
if(!exists('data_df')) {
  library(readxl)
  data_df <- read_excel('combined_updated.xlsx')
}

# Calculate max for x-axis breaks
max_buyers <- max(data_df$num_buyers, na.rm = TRUE)

# Build the plot
buyer_count <- ggplot(data_df, aes(x = num_buyers)) +
  geom_histogram(binwidth = 1, color = 'white', fill = '#3182bd', alpha = 0.85) +
  geom_text(stat = 'bin', binwidth = 1, aes(label = ..count..), vjust = -0.3, size = 3.2, color = '#333333') +
  scale_x_continuous(breaks = seq(0, max_buyers, by = 1)) +
  labs(
    title = 'Distribution of Buyer Presence at Harvest',
    subtitle = 'Number of buyers observed by farmers during apple harvest',
    x = 'Number of Buyers',
    y = 'Frequency (farms)'
  ) +
  theme_minimal(base_size = 12) +
  theme(
    plot.title = element_text(face='bold', size = 12, color = '#222222'),
...
# Show quick peek at num_buyers column
print(head(data_df[['num_buyers']]))

# Save the plot
ggsave(filename = here("figures", "buyer_count_distribution.png"), plot = buyer_count, width = 6, height = 3, dpi = 300)
```
