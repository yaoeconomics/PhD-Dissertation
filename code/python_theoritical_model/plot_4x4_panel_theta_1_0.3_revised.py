...
second-period buyer-power shock theta_2, with FIRST-PERIOD buyer power theta_1 = 0.3.
...
MODEL is unchanged from the original theta_1 = 0.3 script (same constants, same
CRRA objective, same corner (bang-bang) solution at gamma = 0). Only the plotting
is revised.

Outputs (into $OUTPUT_DIR, else ../../model_figures):
    3D_formulation_0.3.png            3D surfaces
    2D_contour_formulation_0.3.png    contour alternative
"""
...
# ---- model constants (unchanged: theta_1 = 0.3) ----
theta1 = 0.3
p1 = 1.0 / (1.0 + theta1)
delta = 1.0
...
fig.suptitle(r"Optimal storage share $s^{*}$ vs. risk aversion $\gamma$ and storage efficiency $\kappa$"
             "\n"
             r"first-period buyer power $\theta_1=0.3$",
...
fig2.suptitle(r"Optimal storage share $s^{*}(\kappa,\gamma)$  ($\theta_1=0.3$)"
              "\n"
...
fig.savefig(os.path.join(target_dir, "3D_formulation_0.3_revised.png"), dpi=300, bbox_inches="tight")
fig2.savefig(os.path.join(target_dir, "2D_contour_formulation_0.3_revised.png"), dpi=300, bbox_inches="tight")
plt.show()
