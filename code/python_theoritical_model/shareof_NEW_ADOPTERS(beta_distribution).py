# -*- coding: utf-8 -*-
"""
3x4 grid: Share of *new adopters* (in %)
Rows:     κ changes in {0.80→0.85, 0.80→0.90, 0.80→0.95} (top → bottom)
Columns:  Beta variance σ² in {0.05, 0.10, 0.15, 0.20} (left → right)

Within each subplot:
- y-axis: Share of *new adopters* (%): farmers with s*≈0 at κ=0.80 who switch to s*>0 at κ=κ_new
...
DPI = 300
FIG_PATH = os.path.join(target_dir, "new_adopters_share_grid_3x4.png")
...
def run_and_plot():
    ...
    fig.suptitle(
        "Share of New Adopters (s* crosses threshold when κ increases)\n"
