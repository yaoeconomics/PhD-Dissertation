...
def run_mu_contrast(mu, k0, k1, nodes_cache, worlds_cache, reuse_worlds):
    ...
    g = df.groupby("decile").agg(
        ...
        # baseline levels under k0 (for percent scaling)
        Epi0=("Y0", "mean"),

        # your existing decomposition
        ΔEπ=("dY", "mean"),
        ΔCE=("dCE", "mean"),
        ΔRP=("dRP", "mean")
    ).reset_index()
...
# ---- Percent gains (relative to baseline expected income Epi0) ----
eps = 1e-12
den = np.maximum(avg_panel["Epi0"].to_numpy(), eps)

avg_panel["ΔEπ_%"] = 100.0 * avg_panel["ΔEπ"].to_numpy() / den
avg_panel["−ΔRP_%"] = 100.0 * avg_panel["−ΔRP (insurance gain)"].to_numpy() / den
avg_panel["ΔCE_%"] = 100.0 * avg_panel["ΔCE"].to_numpy() / den  # optional sanity check: ΔCE_% ≈ ΔEπ_% + −ΔRP_%



# ---- Plot for (2): 1×3 grid, right three κ-contrasts only ----

# Select the right three contrasts (drop the far-left one)
contrasts = avg_panel["contrast"].unique()[1:]
...
for ax, contrast in zip(axes, contrasts):
    ...
    inc_gain = sub["ΔEπ"].to_numpy()
    ins_gain = sub["−ΔRP (insurance gain)"].to_numpy()

    # Stacked bars
    ax.bar(x, inc_gain, label="ΔE[π] (income gain)")
    ax.bar(x, ins_gain, bottom=inc_gain, label="−ΔRP (insurance gain)")

    ax.set_title(f"κ {contrast}", fontsize=20)
    ax.set_xlabel("γ decile", fontsize=18)

    ax.tick_params(axis="both", labelsize=16)
    ax.grid(True, axis="y", linestyle="--", alpha=0.5)

axes[0].set_ylabel("Welfare gain (price units)", fontsize=18)

# Large, slide-friendly legend
...
plt.tight_layout()
plt.savefig(
    os.path.join(target_dir, "storage_subsidy_gain_decomposition_TTU.png"),
    dpi=300,
    bbox_inches="tight"
)
plt.show()
...
# ---- Plot for (2) in percent terms: 1×3 grid, right three κ-contrasts only ----

contrasts = avg_panel["contrast"].unique()[1:]  # right three
...
for ax, contrast in zip(axes, contrasts):
    ...
    inc_gain_pct = sub["ΔEπ_%"].to_numpy()
    ins_gain_pct = sub["−ΔRP_%"].to_numpy()

    # Stacked bars (percent)
    ax.bar(x, inc_gain_pct, label="income gain in %")
    ax.bar(x, ins_gain_pct, bottom=inc_gain_pct, label="insurance gain in %")
...
axes[0].set_ylabel("Welfare gain (% of baseline expected income)", fontsize=18)
...
plt.tight_layout()
plt.savefig(
    os.path.join(target_dir, "storage_subsidy_gain_decomposition_TTU_percent.png"),
    dpi=300,
    bbox_inches="tight"
)
plt.show()
