...
def delta_income_farmer(theta1, gamma, kappa, t2_nodes):
    """
    Income gain for one farmer: E_{theta2}[income(s*)] - p1.
    s* is solved via EU maximisation so risk aversion still affects s*,
    but the reported metric is expected income gain, not CE gain.
    theta2 integrated via quadrature (t2_nodes, w_nodes shared globals).
    """
    p1 = 1.0 / (1.0 + theta1)
    ...
    if abs(gamma) < 1e-12:
        ...
        return (1.0 - s_star) * p1 + s_star * kappa * E_p2 - p1
    ...
    E_income = float(np.dot(inc, w_nodes))
    return E_income - p1


def village_avg_income_pct(mu, sigma2, kappa, gammas):
    """
    Village-average income gain as % of mean no-storage income E[p1].
    ...
    for k, t1 in enumerate(t_nodes):
        farmer_dce = np.array([
            delta_income_farmer(t1, gamma_i, kappa, t_nodes)
...
for r_idx, kappa in enumerate(kappa_rows):
    for c_idx, sigma2 in enumerate(sigma2_cols):
        ...
        for v_idx, (gammas, *_) in enumerate(village_profiles):
            vals = np.array([
                village_avg_income_pct(mu, sigma2, kappa, gammas)
                for mu in mu_feas
            ])
...
# ----------------------------- Figure layout (2 rows × 3 cols + top strip)
...
gs_top  = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=gs_outer[0], wspace=0.42)
...
for v_idx, (title, color, pdf_fn, mean_v) in enumerate(dist_info):
    ...
    if pdf_fn is None:
        ...
        if v_idx == 0:   # Uniform: flat PDF, move label lower
            lbl_x, lbl_y = mean_v + 0.13, y.max() * 0.45
        elif v_idx == 2: # Left-skewed Beta: peak is on right, place label to the left
    ...
    ax.set_ylabel("Density" if v_idx == 0 else "", fontsize=FS_LABEL)  # V2 is now first strip panel
...
legend_handles = [
    ...
    for _, c, ls, lw, lbl in village_profiles
]
fig.legend(
