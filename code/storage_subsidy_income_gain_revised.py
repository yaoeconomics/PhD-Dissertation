# -*- coding: utf-8 -*-
"""
3x3 grid: % Mean Income Gain vs No-Storage
Rows:     κ in {0.95, 0.90, 0.80} (top -> bottom)
Columns:  Beta variance σ² in {0.02, 0.05, 0.10} (left -> right)

Within each subplot:
- y-axis: % mean income gain vs no-storage
- x-axis: μ_{θ1} grid (mean of θ1), step 0.05
- curves: gaps g ∈ {0.00, 0.05, 0.15}, interpreted as μ_{θ2} = μ_{θ1} − g
...
DPI = 300
FIG_PATH = os.path.join(target_dir, "gainpct_grid_3x3.png")
...
CURVE_STYLE = {
    0.00: dict(color="#4D4D4D", marker="o", linewidth=2.0, markersize=3, label=r"$\mu_{\theta_2}=\mu_{\theta_1}$"),
    0.05: dict(color="#2CA02C", marker="s", linewidth=2.0, markersize=3, label=r"$\mu_{\theta_2}=\mu_{\theta_1}-0.05$"),
    0.15: dict(color="#006D2C", marker="^", linewidth=2.0, markersize=3, label=r"$\mu_{\theta_2}=\mu_{\theta_1}-0.15$"),
}
...
def run_and_plot():
    ...
    # Plotting — now 3x3 grid
    fig, axes = plt.subplots(len(kappa_rows), len(sigma2_cols),
                             figsize=(9, 9), sharex=False, sharey=True)
    ...
    for r_idx, kappa in enumerate(kappa_rows):
        for c_idx, sigma2 in enumerate(sigma2_cols):
            ...
            # Titles & labels
            if r_idx == 0:
                ax.set_title(rf"$\sigma^2_{{\theta}}={sigma2:.2f}$", fontsize=14, pad=6)
            if c_idx == 0:
            ...
            ax.set_xlabel(r"$\mu_{\theta_1}$ (mean of $\theta_1$)", fontsize=14)
    ...
    # --- Layout: size the panels first, then park the title and legend
    # directly above them. Measuring the panels avoids hard-coded figure
    # fractions, which break whenever figsize or the row count changes.
    fig.tight_layout(rect=[0.04, 0.06, 0.98, 0.98])
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()
    inv = fig.transFigure.inverted()

    panels_top = max(
        ax.get_tightbbox(renderer).transformed(inv).y1 for ax in axes.flat
    )

    leg = fig.legend(
        handles=legend_handles,
        loc="lower center",                       # anchor the legend's BOTTOM
        bbox_to_anchor=(0.5, panels_top + 0.012), # small, fixed gap above panels
        ncols=3,
        frameon=True,
        fontsize=14,
        handlelength=2.5,
    ...
    fig.canvas.draw()
    leg_top = leg.get_window_extent(renderer).transformed(inv).y1

    fig.suptitle(
        "% Mean Income Gain vs No-Storage\n"
        r"Rows: $\kappa$ $\in$ {0.95, 0.90, 0.80}; "
        r"Cols: $\sigma^2_{\theta}$ $\in$ {0.02, 0.05, 0.10}",
        fontsize=15, y=leg_top + 0.015, va="bottom"
    )
